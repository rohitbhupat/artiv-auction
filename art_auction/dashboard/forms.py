...
class ArtworkCreateForm(forms.ModelForm):
    ...
    class Meta:
        model = Artwork
        fields = [
            "sale_type", "product_name", "product_price", "opening_bid", "product_qty",
...
class ArtworkUpdateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "product_qty": forms.NumberInput(attrs={
                "readonly": "readonly",
                "value": 1,
                "style": "cursor: not-allowed; background-color: #f9f9f9;"
            }),
        }
    ...
    def clean_product_qty(self):
        # Ensure it always remains 1 even if someone tries to modify it via dev tools
        return 1